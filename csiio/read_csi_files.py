...
BASEDATE = datetime.datetime(year=1990,
                             month=1,
                             day=1,
                             hour=0,
                             second=0,
...
def _coerce_timestamp_index(values):
    ...
    missing = parsed.isna()
    if missing.any():
        parsed.loc[missing] = pd.to_datetime(pd.Series(values)[missing],
                                             format='%Y-%m-%d %H:%M:%S',
                                             errors='coerce')
...
def read_csi_formats(csformat):
    ...
    for fmt in csformat:
        if fmt.startswith('ASCII'):
            ...
            _emit('Warning: The format code ' + fmt + ' is not known \n'
...
def _data_to_dataframe(data, meta, filetype, sortindex=True):
    ...
    if len(data) < len(cols):
        raise ValueError(f'Metadata/data column mismatch: expected {len(cols)} columns, got {len(data)}')
    ...
    dataframe[record_col_name] = pd.to_numeric(dataframe[record_col_name], errors='coerce').astype('Int64')
    return dataframe


def _read_csi_files_impl(filename,
                        metaonly=False,
                        quiet=True,
                        sortindex=True,
                        **kwargs):
    ...
    if requested_as_dataframe is False and not quiet:
        _emit('Ignoring asdataframe=False: read path is DataFrame-only.', level='warning', quiet=quiet)

    if isinstance(filename, list):
        results = [_read_csi_files_impl(file,
                                       metaonly=metaonly,
                                       quiet=quiet,
                                       sortindex=sortindex,
                                       **kwargs)
    ...
    with open(filename, mode = 'rb') as file_obj:
        ...
        if '<?xml' in firstline[0]:
            ...
            if csixml[0] != 'csixml':
                if not quiet:
                    _emit('Filecontent indicated XML but apparently it\'s not a csixml file', quiet=quiet)
                return False, False
            else:
                csixmlversion = float(csixml[1].split('=')[-1].strip('"\''))
                if csixmlversion > 1.0:
                    _emit('This reader has been written for CSIXML version 1.0, but the version is ' + str(csixmlversion),
                          level='warning')
        ...
        if filetype in ['TOA5', 'TOB1', 'TOACI1', 'TOB3', 'CSIXML']:
            ...
            if filetype in ['TOA5', 'TOACI1']:
                data = read_csi_toa5(file_obj, meta,
                                    **kwargs)
                ...
                data = read_csi_csixml(file_obj, meta,
                                      **kwargs)
            return data, meta
...
class CSIDataFile:
    ...
    def read(self,
        ...
        if self.data is not None and self.paths:
            ...
            result = _read_csi_files_impl(input_path,
                                         metaonly=metaonly,
                                         quiet=quiet,
                                         sortindex=sortindex,
                                         **kwargs)
        ...
        if not self.paths:
            raise ValueError('No input path or data configured. Set CSIDataFile.paths, pass data, or set CSIDataFile.data.')
    ...
    def convert(self, output_file, output_format, quiet=True):
        ...
        if not self.paths:
            raise ValueError('No input path or data configured. Set CSIDataFile.paths or pass data during initialization.')

        if len(self.paths) == 1:
            return _convert_csi_file_impl(self.paths[0],
                                         output_file,
                                         output_format,
                                         quiet=quiet)
        return _convert_csi_files_impl(self.paths,
                                      output_file,
                                      output_format,
                                      quiet=quiet)
    ...
    def to_csv(self,
        ...
        if self.data is None:
            ...
            if self.paths:
                ...
                raise TypeError('No data loaded for CSV export. Set CSIDataFile.data or call read().')
...
def read_csi_files(filename,
                  metaonly=False,
                  quiet=True,
                  sortindex=True,
                  **kwargs):
    return _read_csi_files_impl(filename,
                               metaonly=metaonly,
                               quiet=quiet,
                               sortindex=sortindex,
                               **kwargs)
...
def read_csi_meta(file_obj, filetype):
    ...
    if metalines >= 0:
        meta = [file_obj.readline().rstrip().decode().split(',')
        ...
        field_nodes = [] if source_node is None else [
            node for node in list(source_node) if node.tag.lower().endswith('field')
        ]
...
def read_csi_convert_tob1_daterec(daterec, units):
    timedeltas = [pd.to_timedelta(daterec[unit_no], unit=unit)
...
def read_csi_csixml(file_obj, meta, guesstype=False):
    ...
    for record in root_children[1]:
        entry = [record.attrib['time'], record.attrib['no'], ]
    ...
    record_col_name = [c for c in dataframe.columns if c.upper() == 'RECORD' or c.upper().startswith('RECORD (')]
    if record_col_name:
        dataframe[record_col_name[0]] = pd.to_numeric(dataframe[record_col_name[0]], errors='coerce').astype('Int64')
...
def read_csi_tob3(file_obj, meta,
                 quiet=True,
                 ):
    csformat = meta[-1]
    pyformat = read_csi_formats(csformat)
    ...
    if multiplier[0].isalpha():
        if len(multiplier) > 3:
            if multiplier[0] in multiplier_scale_dict:
                ...
                _emit(f'warning, length indicates a multiplier_scale ({multiplier[0]}), but none found',
                      level='warning')
                prescale = 1.0
        else:
            if not quiet:
    ...
    while file_obj.tell() <= last_frame_start:
        ...
        if not binary_fhdr or len(binary_fhdr) < fhdrsize:
            # end of file reached
            if not quiet:
                _emit(f'TOB3: unexpected EOF reading frame header {fhdrsize} {len(binary_fhdr)}', quiet=quiet)
            break
                ...
                for ii in range(n_rec_frame):
                    ...
                    for iii in pyformat:
                        ...
                        if iii[-1] == 's' and isinstance(one_record, bytes):
                            one_record = one_record.decode('ascii', errors='ignore').rstrip('\x00').rstrip('\r')
...
def write_csi_tob1(outfile,
    ...
    header = [
        ...
        ','.join('""' for _ in ['SECONDS', 'NANOSECONDS', 'RECORD']) + (',' if payload_names else '') + ','.join('"Smp"' for _ in payload_names),
...
def _convert_csi_file_impl(input_file,
                          output_file,
                          output_format,
                          quiet=True):
    output_format = output_format.upper()
    output_file = _normalize_output_path(output_file, output_format)
    data, meta = read_csi_files(input_file,
                               quiet=quiet)
...
def _convert_csi_files_impl(input_files,
                           output_dir,
                           output_format,
                           quiet=True):
    ...
    for input_file in input_files:
        ...
        outputs.append(convert_csi_file(input_file,
                                       outfile,
                                       output_format,
                                       quiet=quiet))
    return outputs
...
def convert_csi_file(input_file,
                    output_file,
                    output_format,
                    quiet=True):
    return _convert_csi_file_impl(input_file,
                                 output_file,
                                 output_format,
                                 quiet=quiet)


def convert_csi_files(input_files,
                     output_dir,
                     output_format,
                     quiet=True):
    return _convert_csi_files_impl(input_files,
                                  output_dir,
                                  output_format,
                                  quiet=quiet)
